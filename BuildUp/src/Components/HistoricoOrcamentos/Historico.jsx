...
export default function Historico({ onSelecionarOrcamento }) {
  ...
  async function carregarHistorico() {
    try {
      ...
      const response = await api.get(`/Orcamentos/usuario/${idUsuario}`);
  ...
  async function excluirOrcamento(id) {
    try {
      await axios.delete(`http://localhost:5246/api/Orcamentos/${id}`);

      setHistorico((prev) => prev.filter((item) => item.id_Orcamento !== id));
  ...
  return (
    <>
      <div className="historico-card">
        <h3>Histórico de Orçamentos</h3>
        {erroCarregamento ? (
          <p className="erro-historico">Ainda não tem nenhum histórico.</p>
        ) : historico.length === 0 ? (
